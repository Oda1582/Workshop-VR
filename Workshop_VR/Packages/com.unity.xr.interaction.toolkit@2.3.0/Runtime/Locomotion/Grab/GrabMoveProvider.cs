...
{
    if (!m_GrabMoveAction.action.IsPressed())
        return false;
    ...
    foreach (var interactor in m_ControllerInteractors)
    {
        ...
        {
            if (directInteractor.selectTarget != null && directInteractor.selectTarget.gameObject.layer == LayerMask.NameToLayer("Environnement Grabbable"))
                return true;
        }
        else if (interactor is XRSocketInteractor socketInteractor)
        {
            if (socketInteractor.selectTarget != null && socketInteractor.selectTarget.gameObject.layer == LayerMask.NameToLayer("Environnement Grabbable"))
                return true;
        }
    }
